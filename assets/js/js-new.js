...
function loadMenu(){
  ...
  var listGame = [];
    fetch(`/assets/data/menu.json?v=${date_tmp}`).then(response => response.json())
    .then(data => {
      data.forEach(data_tmp => {

        var item = document.createElement('li');
        item.classList.add('navover');
        item.innerHTML = `<a href="/${data_tmp.slug}" class="border0">${data_tmp.title}</a>`
...
function loadCat(cat){
    var date_tmp =  Date.now();
    fetch(`/assets/data/${cat}.json?v=${date_tmp}`).then(response => response.json())
    .then(data => {
        var listGame = [];
        var listCheck = [];
        data.forEach(item => {
        ...
        listGame.forEach((item, index) => {
            ...
            <a href="/${item.slug}"><img src="${img}" width="190" height="190" alt="${item.title}" />
            <div class="capinfos">
...
function loadIo(){
    var date_tmp =  Date.now();
    fetch(`/assets/data/update.json?v=${date_tmp}`).then(response => response.json())
    .then(data => {
        var listGame = [];
        var listCheck = [];
        data.forEach(item => {
...
function loadNew(){
    var date_tmp =  Date.now();
    fetch(`/assets/data/io.json?v=${date_tmp}`).then(response => response.json())
    .then(data => {
        var listGame = [];
        var listCheck = [];
        data.forEach(item => {
...
function loadShoot(){
  var date_tmp =  Date.now();
  fetch(`/assets/data/shooting.json?v=${date_tmp}`).then(response => response.json())
  .then(data => {
      var listGame = [];
      var listCheck = [];
      data.forEach(item => {
...
function loadAction(){
  var date_tmp =  Date.now();
  fetch(`/assets/data/action.json?v=${date_tmp}`).then(response => response.json())
  .then(data => {
      var listGame = [];
      var listCheck = [];
      data.forEach(item => {
